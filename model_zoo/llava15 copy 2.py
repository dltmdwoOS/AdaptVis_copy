...
class LlavaWrapper:
    ...
    def bidirectional_reasoning_single(self, image, question, option_list):
        ...
        prompt_step2a = f"USER: <image>\n{query_step2a} ASSISTANT:"
        ...
        probs_forward = [torch.softmax(s[0], dim=-1).max().item() for s in output.scores]
        ...
        prompt_step2b = f"USER: <image>\n{query_step2b} ASSISTANT:"
        ...
        probs_reverse = [torch.softmax(s[0], dim=-1).max().item() for s in output.scores]
    ...
    def adaptvis_bidirectional_reasoning(self, image, question, option_list, threshold, weight1, weight2, sample_idx=None):
        ...
        """
        reasoning_chain = []
        debug_log = []  # 디버깅 정보를 저장할 리스트
        
        # 디버깅 로그 추가 헬퍼 함수
        def add_log(message, level="INFO"):
            debug_log.append({"level": level, "message": message})
        
        add_log("="*80)
        add_log("AdaptVis-Bidirectional Reasoning Started")
        add_log("="*80)
        ...
        if is_formatted_prompt:
            ...
            question = question.replace('<image>', '').replace('USER:', '').replace('ASSISTANT:', '').strip()
            add_log(f"Original Question (formatted): {original_question}")
            add_log(f"Cleaned Question: {question}")
        else:
            add_log(f"Question: {question}")
        ...
        if obj1 is None or obj2 is None:
            add_log("Failed to extract objects from question!", "ERROR")
            add_log("Falling back to direct query")
            result = self._direct_query_fallback(image, question, option_list, reasoning_chain)
            self._save_debug_log(debug_log, reasoning_chain, sample_idx)
            return result
        
        add_log(f"Successfully extracted objects:")
        add_log(f"  • Object 1 (Subject): '{obj1}'")
        add_log(f"  • Object 2 (Reference): '{obj2}'")
        ...
        add_log("Image token positions identified")
        
        # Weight=1.0으로 기본 inference
        add_log("Generating with baseline weight=1.0 (no scaling)...")
        change_greedy_to_add_weight()
        ...
        add_log(f"  • Response: '{baseline_response}'")
        add_log(f"  • Uncertainty (1st token max prob): {uncertainty:.4f}")
        
        # Threshold 기반 weight 선택
        if uncertainty < threshold:
            selected_weight = weight1
            weight_reason = 'low_confidence'
            add_log("\nWeight Selection Decision:")
            add_log(f"  • LOW confidence detected")
            add_log(f"  • Model is uncertain (prob={uncertainty:.4f} < threshold={threshold})")
            add_log(f"  • Using weight1={weight1} (smooth attention, explore more)")
        else:
            selected_weight = weight2
            weight_reason = 'high_confidence'
            add_log("\nWeight Selection Decision:")
            add_log(f"  • HIGH confidence detected")
            add_log(f"  • Model is confident (prob={uncertainty:.4f} >= threshold={threshold})")
            add_log(f"  • Using weight2={weight2} (sharp attention, focus more)")
        ...
        add_log(f"Query: {query_forward}")
        add_log(f"Weight: {selected_weight}")
        ...
        add_log("\nForward Results:")
        add_log(f"  • Response: '{response_forward}'")
        add_log(f"  • Confidence (avg token prob): {confidence_forward:.4f}")
        add_log(f"  • Token-level probs: {[f'{p:.3f}' for p in probs_forward[:5]]}{'...' if len(probs_forward) > 5 else ''}")
        ...
        add_log("\nReverse Results:")
        add_log(f"  • Response: '{response_reverse}'")
        add_log(f"  • Confidence (avg token prob): {confidence_reverse:.4f}")
        add_log(f"  • Token-level probs: {[f'{p:.3f}' for p in probs_reverse[:5]]}{'...' if len(probs_reverse) > 5 else ''}")
        ...
        # ============================================
        add_log("\nStep 4: Bidirectional Consistency Check & Selection")
        add_log("-" * 60)
        
        # Consistency 검증
        is_consistent = check_logical_consistency(response_forward, response_reverse)
        expected_reverse = get_symmetric_relation(response_forward)
        
        add_log("Consistency Analysis:")
        add_log(f"  • Forward says: '{obj1}' is '{response_forward}' '{obj2}'")
        add_log(f"  • Reverse says: '{obj2}' is '{response_reverse}' '{obj1}'")
        add_log(f"  • Expected reverse: '{expected_reverse}' (symmetric of '{response_forward}')")
        
        # Confidence 차이
        confidence_diff = abs(confidence_forward - confidence_reverse)
        
        add_log("\nConfidence Comparison:")
        add_log(f"  • Forward confidence: {confidence_forward:.4f}")
        add_log(f"  • Reverse confidence: {confidence_reverse:.4f}")
        add_log(f"  • Difference: {confidence_diff:.4f}")
        ...
        if is_consistent:
            ...
            add_log("\nCONSISTENT responses detected!")
            add_log(f"  • Forward '{response_forward}' <-> Reverse '{response_reverse}'")
            add_log(f"  • Logical consistency verified")
            add_log(f"  • Decision: Use forward response")
            add_log(f"  • Final Answer: '{final_prediction}'")
            
        else:
            # 일관성 없음
            add_log("\nINCONSISTENT responses detected!", "WARNING")
            add_log(f"  • Forward: '{response_forward}' (conf: {confidence_forward:.4f})")
            add_log(f"  • Reverse: '{response_reverse}' (conf: {confidence_reverse:.4f})")
            add_log(f"  • Expected: '{expected_reverse}' but got '{response_reverse}'")
            ...
            if confidence_forward >= confidence_reverse:
                ...
                add_log("\nResolution Strategy:")
                add_log(f"  • Forward has HIGHER confidence ({confidence_forward:.4f} >= {confidence_reverse:.4f})")
                add_log(f"  • Decision: Trust forward response")
                add_log(f"  • Final Answer: '{final_prediction}'")
            else:
                # Reverse가 더 확신함 → 대칭 변환
                final_prediction = get_symmetric_relation(response_reverse)
                selected_confidence = confidence_reverse
                selection_reason = 'higher_confidence_reverse_symmetric'
                selected_direction = 'reverse'
                
                add_log("\nResolution Strategy:")
                add_log(f"  • Reverse has HIGHER confidence ({confidence_reverse:.4f} > {confidence_forward:.4f})")
                add_log(f"  • Decision: Trust reverse response (with symmetric transformation)")
                add_log(f"  • Reverse says: '{obj2}' is '{response_reverse}' '{obj1}'")
                add_log(f"  • Symmetric transform: '{obj1}' is '{final_prediction}' '{obj2}'")
                add_log(f"  • Final Answer: '{final_prediction}'")
        ...
        # 최종 요약
        add_log("\n" + "="*80)
        add_log("Final Summary")
        add_log("="*80)
        add_log(f"Original Question: Where is the {obj1} in relation to the {obj2}?")
        add_log(f"Initial Uncertainty: {uncertainty:.4f} ({'LOW' if uncertainty < threshold else 'HIGH'} confidence)")
        add_log(f"Selected Weight: {selected_weight}")
        add_log(f"Forward: '{response_forward}' (conf: {confidence_forward:.4f})")
        add_log(f"Reverse: '{response_reverse}' (conf: {confidence_reverse:.4f})")
        add_log(f"Consistency: {'YES' if is_consistent else 'NO'}")
        add_log(f"Selection: {selected_direction.upper()} ({selection_reason})")
        add_log(f"Final Answer: '{final_prediction}' (conf: {selected_confidence:.4f})")
        add_log("="*80)
    ...
    def _save_debug_log(self, debug_log, reasoning_chain, sample_idx, summary=None):
        """디버깅 정보를 파일로 저장"""
        import os
        import json
